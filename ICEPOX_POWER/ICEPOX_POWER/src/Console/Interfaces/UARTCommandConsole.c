...
#define cmdMAX_MUTEX_WAIT		( 10 / portTICK_PERIOD_MS )
...
#define cmdRXED_CHARS_QUEUE_LENGTH	( 30 )
...
/*
 * Register the stepper motor debugging commands in CLI_motor_commands.c
 */
extern void vRegisterMotorCLICommands( void );
...
static char * const pcWelcomeMessage = "\r\n\r\n *** ICEPOX II THROTTLE BOARD *** \r\nType Help to view a list commands.\r\n\r\n>";
...
void vUARTCommandConsoleStart( uint16_t usStackSize, unsigned portBASE_TYPE uxPriority )
{
	...
	/*some are FreeRTOS standard/examples, others are custom for board debugging*/
...
struct usart_config xUARTConfig;
	...
	xUARTConfig.baudrate    = CONF_STDIO_BAUDRATE;
	xUARTConfig.mux_setting = CONF_STDIO_MUX_SETTING;
	xUARTConfig.pinmux_pad0 = CONF_STDIO_PINMUX_PAD0;
	xUARTConfig.pinmux_pad1 = CONF_STDIO_PINMUX_PAD1;
	xUARTConfig.pinmux_pad2 = CONF_STDIO_PINMUX_PAD2;
	xUARTConfig.pinmux_pad3 = CONF_STDIO_PINMUX_PAD3;

	while( usart_init( pxCDCUsart, CONF_STDIO_USART_MODULE, &xUARTConfig ) != STATUS_OK )
	{
