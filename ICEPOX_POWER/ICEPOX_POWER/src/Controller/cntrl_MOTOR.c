...
+	0x1c,0x00,    //r3
	...
	0xbc,0x0F,    //r23 was 0xbc 0x0f changing to reduce overspeed limit 10-14-19
	0xc4,0x00,    //r24
...
void step(){
	...
	if((gbl_PwrCmd.motor != 0) && (gbl_DigInputs.sw_status == 1)){
		...
		*/
		...
		vTaskDelay(pdMS_TO_TICKS(1000));  // delay waiting for sleep  (DSp, no reason to delay at all, nevermind a whole sec?)
		static uint8_t goToSleep[2] = {0xdd, 0x15};
		motor_send_msg(&goToSleep[0], 1);  // GTS: 0
		vTaskDelay(pdMS_TO_TICKS(1000));  // delay waiting for sleep  (maybe needs 150 usec?)
		goToSleep[0] = 0xdd;
		goToSleep[1] = 0x94;
